...
// ================= GAME PROXY ROUTES (OPTIONAL FALLBACK) =================
// Test endpoint
...
app.get("/proxy/game-sandbox", (req, res) => {
  ...
  // Create sandbox HTML page with GameMonetize iframe script
  // IMPORTANT: Must use id="api_game_embed" for cms_iframe.js to work (same as arcadecms)
  // Use internal proxy to bypass X-Frame-Options restrictions
...
// Proxy for game iframes to bypass X-Frame-Options (FALLBACK ONLY)
// Most games should now load directly via iframe
app.get("/proxy/game", (req, res) => {
  ...
  const proxyReq = client.request(targetUrl, {
    ...
    headers: {
     ...
     // Buffer response to inject jQuery if HTML
     let htmlChunks = [];
     proxyRes.on('data', chunk => htmlChunks.push(chunk));
     proxyRes.on('end', () => {
       let content = Buffer.concat(htmlChunks).toString('utf8');
       const contentType = proxyRes.headers['content-type'] || '';
       
        // Only inject jQuery into HTML responses
        if (contentType.includes('text/html') && !content.includes('jquery-3.6.0.min.js')) {
          // Remove known broken jQuery references (old versions that return HTML errors)
          content = content.replace(/<script[^>]*jquery-3\.4\.1\.min\.js[^>]*><\/script>/gi, '');
          
          // Insert jQuery as first child of <head> to load before any other scripts
          // Use Google CDN for reliability
          const jqueryScript = '<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>';
